...
exports.getAvailableQuestionsForWinner = getAvailableQuestionsForWinner;
exports.addSelectedQuestions = addSelectedQuestions;
exports.useSpecialCard = useSpecialCard;
...
function canPlayCard(game, playerRole, questionId) {
...
function playCard(game, playerRole, questionId) {
    ...
    return (0, gameManager_1.getQuestion)(questionId) || null;
}
// Answer a question
function answerQuestion(game, playerRole, answerIndex) {
    ...
    const question = (0, gameManager_1.getQuestion)(game.activeQuestion.questionId);
    if (!question)
        return null;
...
function getPlayerGameState(game, playerRole) {
    const player = game[playerRole];
    const opponent = playerRole === 'playerA' ? game.playerB : game.playerA;
    if (!player)
        return null;
    ...
    const deckWithQuestions = player.deck.map(questionId => {
        const q = (0, gameManager_1.getQuestion)(questionId);
...
function getAvailableQuestionsForWinner(game, winnerRole) {
...
function addSelectedQuestions(game, winnerRole, questionIds) {
    const winner = game[winnerRole];
    if (!winner)
        return false;
...
// Special Card Logic
function useSpecialCard(game, playerRole, cardType) {
    const player = game[playerRole];
    const opponentRole = playerRole === 'playerA' ? 'playerB' : 'playerA';
    ...
    switch (cardType) {
        ...
        case 'JOKER':
            ...
            if (!opponent)
                return { success: false, message: 'No opponent' };
            // Get random from all questions
            const allQ = (0, gameManager_1.getAllQuestions)();
        ...
        case 'SWAP_SELF':
            ...
            // Add random card
            const allQ2 = (0, gameManager_1.getAllQuestions)();
    ...
    if (idx > -1)
        player.specialHand.splice(idx, 1);
    return { success: true };
}
